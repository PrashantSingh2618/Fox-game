import { modFox, modScene, togglePoopBag, writeModal } from "./transition-ui";
import {
  RAIN_CHANCE,
  SCENES,
  DayLength,
...
const gameState = {
  ...
  tick() {
    this.clock++;
    // console.log("Time is ", this.clock);
  ...
  wake() {
    ...
    // console.log("Next Hungry time is ", this.hungryTime);
  },
  getHungry() {
    this.current = "HUNGRY";
    ...
    modFox("hungry");
    // console.log("Hungry");
    // console.log("Die time is ", this.dieTime);
  },
  startCelebrating() {
  ...
  determineFoxState() {
    if (this.current === "IDLING") {
      if (SCENES[this.scene] === "rain") modFox("idling");
      else modFox("idling");
    }
  },
  poop() {
  ...
  cleanUpPoop() {
    ...
    togglePoopBag(true);
    this.startCelebrating();
    this, (this.hungryTime = getNextHungerTime(this.clock));
  },
  feed() {
    // console.log("feed");
